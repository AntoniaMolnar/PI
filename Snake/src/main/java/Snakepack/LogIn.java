	...
	loginBtn.addActionListener(new ActionListener() {
		...
		String pass=passwordPane.getText();
		 ...
		 try {
			...
			String q=" SELECT user_id FROM [SnakeUser] Where [username] = \'"+usr.toLowerCase()+"\' AND [password] = \'"+pass+"\'";
			...
			System.out.println(q);
				  ...
				  if(dbResponse != null) {int ok=0;
					  while (dbResponse.next()) {ok=1;
						SnakeGame.CurrentUser = dbResponse.getInt("user_id");
						SnakeGame.UserName=usr.toLowerCase();
						frame.setVisible(false);
